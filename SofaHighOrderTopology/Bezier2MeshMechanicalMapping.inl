...
#ifndef SOFA_HIGHORDERTOPOLOGY_BEZIER2MESHMECHANICALMAPPING_INL
#define SOFA_HIGHORDERTOPOLOGY_BEZIER2MESHMECHANICALMAPPING_INL
...
double multinomial(const size_t n,const sofa::component::topology::TriangleIndexVector tbiIn)
{
	size_t i,ival;
	sofa::component::topology::TriangleIndexVector tbi=tbiIn;
...
void Bezier2MeshMechanicalMapping<TIn, TOut>::init()
{
	...
	if (!topoMap) {
		serr << "Could not find any HighOrder2MeshTopologicalMapping object"<<sendl;
		return;
	}
	...
	if (precomputedTriangularBernsteinCoefficientArray.size()!= ((bezierTesselation-1)*(bezierTesselation-2)/2)) {
		...
		Real u,v,w;
		size_t j,k,ind;
		sofa::helper::vector<sofa::component::topology::TriangleIndexVector> tbia=btstc->getTriangleIndexArray();
	...
	if (tesselatedTriangleIndices.size()!=(bezierTesselation+1)*(bezierTesselation+2)/2 ) {
		tesselatedTriangleIndices=btstc->getTriangleIndexArrayOfGivenDegree(bezierTesselation);
	}
	...
	/* 
	  ...
	  // then compute normals if necessary
		  if (m_useBezierNormals.getValue()) {
			  ...
			  for (i=0;i<btstc->getNumberOfTriangles();++i) {
				  // first get  the Bezier control points in the triangle 
				  btstc->getGlobalIndexArrayOfControlPointsInTriangle(i, indexArray);
