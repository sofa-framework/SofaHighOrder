...
#include "HighOrder2MeshTopologicalMapping.h"
...
SOFA_DECL_CLASS ( HighOrder2MeshTopologicalMapping )

// Register in the Factory
int HighOrder2MeshTopologicalMappingClass = core::RegisterObject ( "This class maps a high order tetrahedral or triangular mesh into an interpolated refined tetrahedral or triangular mesh" )
        .add< HighOrder2MeshTopologicalMapping >()
        ;

// Implementation
HighOrder2MeshTopologicalMapping::HighOrder2MeshTopologicalMapping ()
...
void HighOrder2MeshTopologicalMapping::init()
{
//	initDone = true;
	if(!fromModel)
	{
	...
	HighOrderTriangleSetTopologyContainer *btstc;
	fromModel->getContext()->get(btstc);

	if (!btstc) {
		serr << "Could not find an input  HighOrderTriangleSetTopologyContainer " <<sendl;
	}
...
//	toModel->getContext()->get(toTetrahedronMod, sofa::core::objectmodel::BaseContext::Local);
	...
	*/
		 ...
		 sofa::helper::vector< sofa::component::topology::HighOrderTriangleSetTopologyContainer::LocalTriangleIndex> 
		...
		sofa::component::topology::HighOrderTriangleSetTopologyContainer::VecPointID indexArray;
		sofa::helper::vector<size_t>  bezierEdge;
		sofa::component::topology::TriangleIndexVector trbi;
		...
		for (rank=0,i=0;i<btstc->getNumberOfTriangles();++i) {
			...
			Triangle tr=btstc->getTriangle(i);
	...
	//		for(j=0;j<tesselatedTriangleIndices.size();++j)
		//		std::cerr<< (sofa::defaulttype::Vec<3,size_t >)(tesselatedTriangleIndices[j])<<std::endl;
			...
			if (bezierTesselation>1) {
				 typedef sofa::component::topology::HighOrderTriangleSetTopologyContainer::VecPointID VecPointID;
				EdgesInTriangle eit=btstc->getEdgesInTriangle(i);
				const VecPointID &indexArray= btstc->getGlobalIndexArrayOfControlPoints(i);
				for (j=0;j<3;++j) {

					// store the edge in an array only once 
					const TrianglesAroundEdge &tae=  btstc->getTrianglesAroundEdge(eit[j]);
					if (tae[0]==i) {
						const Edge &e= btstc->getEdge(eit[j]);
						...
						for(l=1;l<=(bezierDegree-1);++l) {
							...
							bezierEdge.push_back(indexArray[  btstc->getLocalIndexFromTriangleIndexVector(trbi)]);
						}						  
...
void HighOrder2MeshTopologicalMapping::updateTopologicalMappingTopDown() {
